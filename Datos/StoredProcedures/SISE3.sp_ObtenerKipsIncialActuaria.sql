...
CREATE PROCEDURE [SISE3].[sp_ObtenerKipsIncialActuaria]
...
BEGIN
    ...
    -- Crear y poblar la tabla temporal con los datos necesarios
    CREATE TABLE #TMP_NOTIFICACIONES (
        ...
        Parte VARCHAR(255),              -- Nombre de la parte
        ParteID INT,                     -- Identificador de la parte
        Caracter VARCHAR(255),           -- Carácter de la parte
        DomicilioParte VARCHAR(255),     -- Domicilio de la parte
        ...
        Folio VARCHAR(255),              -- Folio
        NombreArchivo VARCHAR(255),      -- Nombre del archivo
        Guid UNIQUEIDENTIFIER,           -- GUID del archivo
    ...
    SELECT 
        ...
        END AS Parte,                      -- 9. Nombre de la parte
        nep.PersonaId AS ParteID,          -- 10. Identificador de la parte
        cpa.Descripcion AS Caracter,       -- 11. Carácter de la parte
        IIF(dom.ParteId IS NOT NULL, CONCAT(dom.TipoVialidadId, ' ', dom.NombreVialidad, ' ', dom.NumeroExterior, ' INT ', dom.NumeroInterior, ' ', dom.TipoAsentamientoId, ' ', dom.CP, ' ', dom.MunicipioId, ' ', dom.EstadoId), 'Domicilio no registrado') AS DomicilioParte, -- 12. Domicilio de la parte
        CASE
        ...
        ax.Folio,                          -- 23. Folio
        CONCAT(eo.NombreArchivo, eo.ExtensionDocumento) AS NombreArchivo, -- 24. Nombre del archivo
        eo.uGuid AS Guid,                  -- 25. GUID del archivo
    ...
    LEFT JOIN Anexos ax ON nep.PersonaId = ax.AnexoParteId AND ad.AsuntoDocumentoId = ax.AsuntoDocumentoId AND ax.AnexoTipoId = 6
    LEFT JOIN SISE3.EstadoOficio eo ON nep.AsuntoNeunId = eo.AsuntoNeunId AND ax.AnexoId = eo.AnexoId
    ...
    AND nep.FechaAlta BETWEEN @pi_FechaInicial AND @pi_FechaFinal;

    -- Primer conjunto de resultados: Notificaciones pendientes divididas por días
    SELECT 
        CASE 
            WHEN DATEDIFF(DAY, EstadoFecha, GETDATE()) > 3 THEN '+3 días'
            WHEN DATEDIFF(DAY, EstadoFecha, GETDATE()) = 2 THEN '2 días'
            WHEN DATEDIFF(DAY, EstadoFecha, GETDATE()) = 1 THEN '1 día'
            ELSE 'Hoy'
        END AS Dias,
        COUNT(*) AS Total
    FROM
    ...
    -- Segundo conjunto de resultados: Total de notificaciones
    SELECT 
        COUNT(*) AS Total,
    ...
    -- Tercer conjunto de resultados: Notificaciones notificadas divididas por tipo de notificación
    SELECT 
        Tipo,
        COUNT(*) AS Total
    FROM
