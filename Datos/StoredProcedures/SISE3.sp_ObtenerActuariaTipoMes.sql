...
BEGIN
    ...
    SELECT 
        ISNULL(DATENAME(MONTH, FechaAlta), 'SinMes') AS Mes,
        ISNULL(DATEPART(MONTH, FechaAlta), 0) AS NumeroMes,
        Tipo,
        COUNT(*) AS Total
    FROM
        #TMP_NOTIFICACIONES
    GROUP BY
        ISNULL(DATENAME(MONTH, FechaAlta), 'SinMes'),
        ISNULL(DATEPART(YEAR, FechaAlta), 0),
        ISNULL(DATEPART(MONTH, FechaAlta), 0),
        Tipo
    ORDER BY
        ISNULL(DATEPART(YEAR, FechaAlta), 0),
        ISNULL(DATEPART(MONTH, FechaAlta), 0);
