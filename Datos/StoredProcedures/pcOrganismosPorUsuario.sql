...
/****** Object:  StoredProcedure [SISE3].[pcOrganismosPorUsuario]    Script Date: 12/1/2023 6:21:30 PM ******/
SET ANSI_NULLS ON
GO
...
-- Basado en: EXEC [SISE3].[pcOrganismosPorUsuario] 7143
-- =============================================
CREATE PROCEDURE  [SISE3].[pcOrganismosPorUsuario]
      @pi_EmpleadoId int  
AS
BEGIN
      ...
      BEGIN
				  ...
				  SELECT cc.CircuitoOrden, co.CatOrganismoId, co.NombreOficial, co.CatTipoOrganismoId, 
						CASE isnull(a.CargoId, '0') WHEN 0 THEN 'false' ELSE 'true' END AS Visible, 
						dbo.fn_OT_ValidarFechaTurnoXOrganismo(a.CatOrganismoId) AS CatHorarioIngresoValidoId,
						CASE WHEN dbo.fn_OT_ValidarFechaTurnoXOrganismo(a.CatOrganismoId) = 1 THEN 'true' ELSE 'false' END AS TurnoActivo
                  FROM CatEmpleados AS b WITH (nolock) 
						INNER JOIN EmpleadoOrganismo AS a WITH (nolock) ON b.EmpleadoId = a.EmpleadoId 
						INNER JOIN CatOrganismos AS co WITH (nolock) ON a.CatOrganismoId = co.CatOrganismoId 
				  ...
				  FROM dbo.CatEmpleados AS b with(nolock)
					   ...
					   INNER JOIN dbo.HISTOJ_REL_OrganismoCronologia co with(nolock) ON a.CatOrganismoId = co.fkIdCatOrganismoCronologia 
      ...
      BEGIN
		  ...
		  CREATE TABLE #TmpSIN1580n2
           ...
           SELECT cc.CircuitoOrden, co.CatOrganismoId, co.NombreOficial, co.CatTipoOrganismoId, 
            ...
            FROM dbo.CatEmpleados AS b with(nolock)
                  INNER JOIN dbo.EmpleadoOrganismo a with(nolock) ON b.EmpleadoId = a.EmpleadoId 
                  INNER JOIN dbo.CatOrganismos co with(nolock) ON a.CatOrganismoId = co.CatOrganismoId 
