...
-- Ejemplo: EXEC [SISE3].[pcDatosusuario] 6712, 180

CREATE OR ALTER PROCEDURE [SISE3].[pcDatosUsuario]
...
BEGIN
	...
	AND CatOrganismoId = @pi_CatOrganismoId
  	...
  	SELECT  TOP 1 CAST(a.EmpleadoId AS INT) AS EmpleadoId
		   ...
		   ,eo.IdRol
		   ,c.sNombreRol AS CargoDescripcion
		   ,b.CatOrganismoId
		   ,b.NombreOficial
		   ...
		   ,'' as registro -- SBGE 06/03/2017 se obtiene el identificadorpara obtener con este los permisos de archivos en tabla PermisosArchivo
		   ,tc.correo AS EMail 
	FROM dbo.CatEmpleados AS a WITH (NOLOCK) 
	--INNER JOIN dbo.EmpleadoOrganismo AS eo ON a.EmpleadoId = eo.EmpleadoId 
	INNER JOIN SISE3.REL_RolEmpleadoXOrganismo eo on eo.IdCatEmpleado =a.EmpleadoId
	INNER JOIN dbo.CatOrganismos AS b WITH (NOLOCK) ON b.CatOrganismoId = eo.IdOrganismo 
	--INNER JOIN dbo.CatCargo AS c WITH (NoLock) ON eo.CargoId = c.CargoId 
	INNER JOIN SISE3.CatRol as c on eo.IdRol = c.IdRol
	LEFT JOIN dbo.OrganismosTipoAsuntoMaterias AS d ON b.CatOrganismoId = d.CatOrganismoId AND d.StatusReg = 1 
	LEFT JOIN dbo.CatCircuitos AS e ON e.CatCircuitoId = b.CatCircuitoId 
	LEFT JOIN dbo.RolesEmpleados AS re ON re.EmpleadoId = eo.IdCatEmpleado 
