...
{
    public class ObtenerComboAsuntoFunction
    {
        private readonly IHttpRequestProcessor _processor;       
        private readonly ISesionService _sesionService;
        public ObtenerComboAsuntoFunction(IHttpRequestProcessor processor, ISesionService sesion)
        {
        ...
        /// <returns></returns>
        [FunctionName("ObtenerComboAsunto")]
        [OpenApiOperation(operationId: "Run", tags: new[] { "obtenerComboAsunto" }, Summary = " Obtiene el el catalogo de  tipo asunto para consulta de seguimiento")]
        ...
        [OpenApiParameter(name: "TipoAsunto", In = ParameterLocation.Query, Required = true, Type = typeof(string), Description = "Representa el identificador del tipo de asunto")]
        [OpenApiParameter(name: "TipoDocumento", In = ParameterLocation.Query, Required = true, Type = typeof(string), Description = "Representa el tipo documento del numero de expediente")]
        ...
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "ObtenerComboAsunto")] HttpRequest req,
            ILogger log)
        {
            ...
            var seguimiento = new Seguimientos.Seguimiento()
            {
                ...
                TipoAsunto = req.Query["TipoAsunto"],
                TipoDocumento = req.Query["TipoDocumento"],
                TipoProcedimiento = req.Query["TipoProcedimiento"],
                CatOrganismoId = _sesionService.SesionActual.CatOrganismoId
            };
            ObtenerComboAsunto seguimientoComando = new ObtenerComboAsunto();
            seguimientoComando.seguimiento = seguimiento;

            return await _processor.ExecuteAsync<ObtenerComboAsunto, List< Seguimientos.Seguimiento>> (seguimientoComando);
        }
    }
}
