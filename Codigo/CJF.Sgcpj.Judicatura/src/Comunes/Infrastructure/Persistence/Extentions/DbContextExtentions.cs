...
public static class DbContextExtensions
{
    ...
    private static T MapTo<T>(this DbDataReader dr)
    {
        T obj = Activator.CreateInstance<T>();
        var props = typeof(T).GetRuntimeProperties();
    ...
    public static async Task<T> ExecuteStoredProcObj<T>(this DbCommand command)
    {
        using (command)
        {
            if (command.Connection.State == ConnectionState.Closed)
    ...
    public static DbCommand WithSqlParamList(this DbCommand cmd, SqlParameter[] paremeters)
    {
        if (string.IsNullOrEmpty(cmd.CommandText))
            throw new InvalidOperationException(
              "Call LoadStoredProc before using this method");
