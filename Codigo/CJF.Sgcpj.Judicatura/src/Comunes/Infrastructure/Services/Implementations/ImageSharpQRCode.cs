                                 ...
                                 bool drawQuietZones = true)
        {
            ...
            for (var x = 0; x < numModules; x += 1)
            {
                for (var y = 0; y < numModules; y += 1)
                {
                    ...
                    if (!IsPartOfFinderPattern(x, y, numModules, offset) || (drawQuietZones && IsPartOfQuietZone(x, y, numModules)) || IsPartOfFinderPattern(x, y, numModules, offset))
                    {
            ...
            if (logoImage != null)
            {
                var logoSizeX = (int)(size * 0.48);
                var logoSizeY = (int)(size * 0.26);
                var logoOffset = drawQuietZones ? 4 : 0;
                var locationPadding = logoLocation switch
                {
                    LogoLocation.Center => new PointF(size / 2, size / 2),
                    _ => new Point(size - logoSizeX / 2 - logoOffset * pixelsPerModule, size - logoSizeY / 2 - logoOffset * pixelsPerModule),
                };


                var locationLogo = logoLocation switch
                {
                    LogoLocation.Center => new Point(size / 2 - logoSizeX / 2, size / 2 - logoSizeY / 2),
                    _ => new Point(size - logoSizeX - logoOffset * pixelsPerModule, size - logoSizeY - logoOffset * pixelsPerModule),
                };


                IPath backgroundShape = logoBackgroundShape switch
                {
                    LogoBackgroundShape.Circle => new EllipsePolygon(locationPadding, logoSizeX / 2),
                    _ => logoLocation == LogoLocation.Center ? new RectangularPolygon(size / 2 - logoSizeX / 2, size / 2 - logoSizeY / 2, logoSizeX, logoSizeY) : new RectangularPolygon(size - logoSizeX - logoOffset * pixelsPerModule, size - logoSizeY - logoOffset * pixelsPerModule, logoSizeX, logoSizeY),
                };
