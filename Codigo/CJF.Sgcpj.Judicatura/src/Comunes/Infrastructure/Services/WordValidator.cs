...
public class WordValidator : IWordValidator
{
        ...
        using (MemoryStream stream = new MemoryStream(data))
        {
            using (var reader = new BinaryReader(stream))
            {
                try
                {
                    ...
                    else if (pathFile == Constants.SISE3_EXTENSIONWORDFILE)
                    {
                        //var condicion = isDocxValid(data, false) && signatures.Any(signature => headerBytes.Take(signature.Length).SequenceEqual(signature));
                        //Se requiere implementar la validación del documento .doc, ya que por ahora solo funciona para .docx
    ...
    public bool IsNameAndValidatedExtension(string nombreArchivo)
    {
        ...
        var validacion = ExtensionesValidas.Any(ext => nombreArchivo.EndsWith(ext, StringComparison.OrdinalIgnoreCase));
        return validacion;
    }
}
