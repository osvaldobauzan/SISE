        ...
        <template v-slot:label>
          <div class="row items-center no-wrap">
            ...
            <div class="text-left text-secondary">
              <q-item-label caption>{{ props.title }}</q-item-label>
              {{ selectedDateCopy ? selectedDateCopy.from + " - " + selectedDateCopy.to: 'DD/MM/YYYY - DD/MM/YYYY' }}
            </div>
          </div>
        </template>
        <q-list>
          ...
          >
            ...
            <q-item-section>Últimos 3 días</q-item-section>
          </q-item>
          <q-item
            clickable
            v-ripple
          ...
          <q-expansion-item :disable="!filtrarPorFecha">
            ...
            <q-date
              ...
              :options="
                (d) =>
                  d >= '1900/01/01' &&
                  d <= date.formatDate(props.maxSelectDate, 'YYYY/MM/DD')
              "
              @update:model-value="setRangoFechas"
            />
          </q-expansion-item>
          <template v-if="props.isSeguimientoPage">
...
const props = defineProps({
  ...
  rango: {
    ...
    default: function () {
      ...
      const sToday = date.formatDate(today, "DD/MM/YYYY");
      return {
        from: sToday,
        to: sToday,
        filter: true,
...
function setDefault() {
  ...
  selectedDate.value = props.rango;
  esNullFecha();
  emit("update:selectedDate", selectedDate.value);
}
/**
...
/**
 * Establece el filtro de fechas a rango de ultimos 3 dias
 */
function setUltimos3dias() {
  iconDate.value = "mdi-calendar-week";
  selectedDate.value = {
    from: date.formatDate(Date.now() - 1000 * 60 * 60 * 24 * 2, "DD/MM/YYYY"),
    to: date.formatDate(Date.now(), "DD/MM/YYYY"),
    filter: filtrarPorFecha,
  };
  esNullFecha();
